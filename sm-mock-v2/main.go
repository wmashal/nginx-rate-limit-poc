...
func main() {
	...
	r.HandleFunc("/v2/service_bindings", getServiceBindings).Methods("GET")
	r.HandleFunc("/v2/service_bindings", createServiceBinding).Methods("POST")
	r.HandleFunc("/v2/service_bindings/{id}", getServiceBinding).Methods("GET")
	r.HandleFunc("/v2/service_bindings/{id}", deleteServiceBinding).Methods("DELETE")

	// Service Offerings endpoints
	r.HandleFunc("/v2/service_offerings", getServiceOfferings).Methods("GET")
	r.HandleFunc("/v2/service_offerings", createServiceOffering).Methods("POST")
	r.HandleFunc("/v2/service_offerings/{id}", getServiceOffering).Methods("GET")
	r.HandleFunc("/v2/service_offerings/{id}", deleteServiceOffering).Methods("DELETE")

	// Service Plans endpoints
	r.HandleFunc("/v2/service_plans", getServicePlans).Methods("GET")
	r.HandleFunc("/v2/service_plans", createServicePlan).Methods("POST")
	r.HandleFunc("/v2/service_plans/{id}", getServicePlan).Methods("GET")
	r.HandleFunc("/v2/service_plans/{id}", deleteServicePlan).Methods("DELETE")

	// Service Instances endpoints
	r.HandleFunc("/v2/service_instances", getServiceInstances).Methods("GET")
	r.HandleFunc("/v2/service_instances", createServiceInstance).Methods("POST")
	r.HandleFunc("/v2/service_instances/{id}", getServiceInstance).Methods("GET")
	r.HandleFunc("/v2/service_instances/{id}", deleteServiceInstance).Methods("DELETE")
	...
	port := os.Getenv("PORT")
	//if port == "" {
	//	port = "8999" // fallback to original port if not set
	//}

	log.Printf("Starting Service Manager Mock V2 on :%s", port)
